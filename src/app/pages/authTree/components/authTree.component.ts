...
export class AuthTreeComponent {
  ...
  constructor(private zone:NgZone) {  }
    ...
    private checkChange(target:PermissionEffectInfo){

      if (this.menuItem.deny){

        this.menuItem.allow = false;
        this.menuItem.deny = false;

        let changedTarget = new PermissionEffectInfo(this.menuItem.id, this.menuItem.moduleId, "[empty]");
        let result = new PermissionEmit("effect.changed", changedTarget)
        this.eventNotify(result);
      }
      else if(this.menuItem.allow){

        if(target.effect == "[empty]"){
