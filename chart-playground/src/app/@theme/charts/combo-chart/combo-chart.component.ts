...
export class ComboChartComponent extends BaseChartComponent  {
  ...
  update(): void {
    ...
    this.yScaleLine = this.getYScaleLine(this.getYDomain(), this.dims.height);
  ...
  setDataList(){
    ...
    this.results.forEach(
      (data) => {
        if(data.value > scaleMax) scaleMax = data.value;
      }
    );

    let lineMax = 0;
    this.lineChart[0].series.forEach(
      (data) => {
        if(data.value > lineMax) lineMax = data.value;
      }
    );
    ...
    if(scaleMax > lineMax){
      ...
      return this.lineChart[0].series
    }
  }
