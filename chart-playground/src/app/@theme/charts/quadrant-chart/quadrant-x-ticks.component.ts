...
@Component({
    selector: 'g[ngx-quadrant-x-ticks]',
    template: `
    <svg:g #ticksel>
    ...
    <svg:g *ngFor="let tick of ticks;"
        [attr.transform]="tickTransform(tick)">
        <svg:g *ngIf="!quadrantSwitch && showGridLines"
    ...
    <svg:g
        [attr.transform]="directionTransform('up')">
        <svg:g>
            <svg:line
                style="stroke:black"
                class="gridline-path gridline-path-vertical"
    ...
    <svg:g
        [attr.transform]="directionTransform('down')">
        <svg:g>
            <svg:line
                style="stroke:black"
                class="gridline-path gridline-path-vertical"
...
export class QuadrantXTicksComponent implements OnChanges, AfterViewInit {
    // property
    ...
    directionTransform(directrion: string): string{
        ...
        if(this.quadrantSwitch){
            yOffset = -(this.gridLineHeight/2)-5;
        }else {
            yOffset = -5;
        }

        return 'translate(0,' + yOffset + ')' + ' ' + 'rotate(' + angle + ',' + this.width + ',0)';
    }
}